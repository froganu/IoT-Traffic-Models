...
def main():
    """Run all Phase 1 + Phase 2 integration tests."""
    phase1_phase2_dir = Path(__file__).parent
    
    # Find all test scripts (excluding __init__.py and this file)
    test_scripts = sorted([
        f for f in phase1_phase2_dir.glob('test_*.py')
        if f.name != 'run_all_tests.py'
    ])
    
    if not test_scripts:
        print("No test scripts found in phase1_phase2/")
        return 1
    
    print("=" * 80)
    print("RUNNING ALL PHASE 1 + PHASE 2 INTEGRATION TESTS")
    print("=" * 80)
    ...
    # Summary
    print("=" * 80)
    print("PHASE 1 + PHASE 2 INTEGRATION TEST SUMMARY")
    print("=" * 80)
    print()
